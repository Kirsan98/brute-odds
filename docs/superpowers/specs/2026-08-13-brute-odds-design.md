...
mesure réelle de `scripts/bench.ts` (`npm run bench`) : ≈ 0,046 ms par combat (moyenne de
5 exécutions de 2000 combats, après une chauffe de 200 combats), mesuré sur un Intel Core
...
ne consomment qu'environ 0,55 s — c'est donc le plafond de 2000 qui fixe N, pas le budget
...
environ 3,6 fois plus coûteux (~0,167 ms) avant que le budget de 2 s redevienne la
